...
import { saveDeviceToken } from '@/lib/push/pushServer'
 ...
 *
 * Any signed-in account may register its own device. The uid always comes
 * from the verified ID token, never from the body, so nobody can attach a
 * device to someone else's account. Fails closed behind FCM_ENABLED.
 */
export async function POST(request: Request) {
  ...
  const rateLimit = checkRateLimit(`push-register:${uid}`, RATE_LIMIT_MAX, RATE_LIMIT_WINDOW_MS)
  ...
  try {
    await saveDeviceToken(firestore, uid, body.token)
    return NextResponse.json({ code: 'ok' }, { headers: noStore })
  } catch (error) {
    console.error('push/register failed:', error instanceof Error ? error.message : String(error))
    return NextResponse.json({ code: 'register-failed' }, { status: 500, headers: noStore })
  }
}
