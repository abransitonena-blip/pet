  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="rounded-2xl border p-6 space-y-4" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
        ...
        <button type="button" onClick={submitRequest} disabled={submitting} className="btn-primary min-h-11 inline-flex items-center gap-2">{submitting && <Loader2 className="animate-spin" size={15} />}{submitting ? 'Registrando…' : 'Registrar solicitud'}</button>
      </div>
    </div>
  )
}
