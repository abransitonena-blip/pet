    ...
    <div className="glass-card p-5 sm:p-6">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-white">Mis reservas</h3>
        ...
        <span className="text-xs text-white/30">{collapsed ? 'Expandir' : 'Cerrar'}</span>
      </button>

      {loading && (
        <div className="flex items-center justify-center gap-2 py-6">
          <FaSpinner className="animate-spin text-primary" size={14} />
          <span className="text-xs text-white/30">Cargando reservas...</span>
        </div>
      )}

      {!loading && searched && reservations.length === 0 && (
        <p className="text-xs text-white/30 text-center py-4">No hay reservas con este número</p>
      )}
      ...
      {!loading && searched && reservations.length > 0 && !collapsed && (
        ...
        >
          {active.length > 0 && (
            <>
              <p className="text-[10px] uppercase tracking-wider text-white/20 font-medium">Próximos paseos</p>
              {active.map((r) => (
                <motion.div
                  key={r.id}
              ...
              {past.slice(0, 5).map((r) => (
                <div key={r.id} className="flex items-center justify-between py-1.5">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="text-xs text-white/60 truncate">{r.petName} · {SERVICE_LABELS[r.service] || r.service}</span>
                  </div>
      ...
      {!searched && !loading && (
        <p className="text-xs text-white/30 text-center py-2">Ingresa tu teléfono en el formulario para ver tus reservas</p>
      )}
    </div>
  )
}
