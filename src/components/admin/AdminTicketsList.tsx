...
import { looksLikeShortFolio, matchesShortFolio, shortFolio } from '@/lib/ticketFolio'
...
export default function AdminTicketsList() {
  ...
  useEffect(() => {
    let active = true
    // Un folio corto no existe en la base: es el final del largo. Con el corto
    // se listan los recientes y se filtra aquí; con el largo, consulta exacta.
    const typed = exactFolio.trim()
    void listTickets(typed && !looksLikeShortFolio(typed) ? { exactFolio: typed } : undefined).then((items) => {
      if (!active) return
      setTickets(items)
      setState('ready')
    }).catch((error: unknown) => {
  ...
  const filtered = useMemo(() => {
    ...
    return tickets.filter((ticket) => {
      if (dateFilter && dateValue(ticket.createdAt) !== dateFilter) return false
      if (typed && looksLikeShortFolio(typed) && !matchesShortFolio(ticket.folio, typed)) return false
      return true
    })
  }, [dateFilter, exactFolio, tickets])
  ...
  return (
    <main className="mx-auto w-full max-w-6xl space-y-5 overflow-x-hidden">
      ...
      <form className="grid gap-3 border-b border-ink/10 pb-5 sm:grid-cols-[minmax(0,1fr)_180px_auto]" onSubmit={(event) => { event.preventDefault(); search() }}>
        <label className="text-sm font-semibold text-ink">Folio<Input className="mt-2" value={exactFolio} onChange={(event) => setExactFolio(event.target.value)} placeholder="AB12CD o TKT-…" /><span className="mt-1 block text-xs font-normal text-muted">El código corto del ticket impreso, o el folio completo.</span></label>
